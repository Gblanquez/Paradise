...
const SELECTORS = {
  ...
  contentLink: '.work-content-link',
  current: '.work-current',
  length: '.work-length',
  title: '[data-a="title-work"]',
...
export function initWorkCarrousel() {
  ...
  const contentContainer = container.querySelector(SELECTORS.contentContainer)
  ...
  const contentLinks = contentItems.map((item) => item.querySelector(SELECTORS.contentLink) || item)
  const titles = contentItems.map((item) => item.querySelector(SELECTORS.title) || item)
  ...
  const setContentAutoAlpha = contentItems.map((item) => gsap.quickSetter(item, 'autoAlpha'))
  const setContentX = contentLinks.map((link) => gsap.quickSetter(link, 'xPercent'))
  const setTitleClipPath = titles.map((title) => gsap.quickSetter(title, 'clipPath'))
  ...
  const render = ({ scroll }) => {
    ...
    contentItems.forEach((item, index) => {
      ...
      setContentAutoAlpha[index](isVisible ? 1 : 0)

      if (isIncoming) {
        const reveal = distance * 100

        setContentX[index](distance * 8)
        setTitleClipPath[index](`inset(0 0 0 ${reveal}%)`)
      ...
      if (isOutgoing) {
        setContentX[index](distance * 3)
        setTitleClipPath[index]('inset(0 0 0 0%)')

        item.style.zIndex = String(contentItems.length)
        return
      }

      setContentX[index](0)
      setTitleClipPath[index]('inset(0 0 0 100%)')
      item.style.zIndex = '0'
    })
  }
  ...
  return () => {
    ...
    gsap.set(contentContainer, { clearProps: 'pointerEvents' })
    gsap.set(contentList, { clearProps: 'height' })
    ...
    gsap.set(contentItems, { clearProps: 'opacity,visibility,willChange,zIndex,left,top,width,height' })
    gsap.set(contentLinks, { clearProps: 'display,width,height,willChange,transform' })
    gsap.set(titles, { clearProps: 'display,overflow,willChange,clipPath' })
  }
}
